...
class Core {
  ...
  public validate(): Promise<string> {
    ...
    return this.filter
      ...
      .then(() => {
        return Promise.all(
          Object.keys(this.fields).map((field) => this.validateField(field))
        ).then((results) => {
  ...
  public executeValidator(
    ...
    const willValidate = this.filter.execute('field-should-validate', true, [
      field,
      ele,
      value,
      v,
  ...
  public updateValidatorOption(
    ...
    if (
      ...
      ;(this.fields[field].validators[validator] as ValidateOptions)[name] =
        value
    }
